...
#####  十、SpringCloud Alibaba Seata分布式事务解决方案

######  《前言》：跟着老师搭建环境，Codeing 代码，老师用的是 0.9.0 版本，我从官网一看，最新的是1.1.0。我一想，为啥不用1.1.0 呢？于是，我就下载了1.1.0 版本，下载解压，修改配置文件，启动，完全OK。于是觉得不会有什么问题，就开始编码。结果，你应该可以想到发生了什么o(╥﹏╥)o。于是不得不重新下载0.9.0 版本，再次试验，成功^_^

1. 首先Seata官网下载tar 包，解压，之后修改 file.conf 配置文件和registry.conf 配置文件。
2. 修改完成以后，启动Nacos ，再启动Seata ，在 Nacos Web 管理界面可以看到Seata 已经注册进Nacos
3. 创建三个微服务对应的数据库和三张对应的业务表
4. 创建seata-order-service2001 微服务，提供下单接口，在 order 微服务中开启 @GlobalTransactional 注解。
5. 创建seata-storage-service2002 微服务，为 order 微服务提供扣减库存接口
6. 创建seata-account-service2003 微服务，为 order 微服务提供扣减余额接口
7. 通过上述步骤，可以发现，只要是有一个服务发生了异常，那么整个数据就不会插入到数据库中

后续：1.1.0 版本的最终还是没有搞出来，等以后慢慢研究研究官网，再往出捣鼓。

原理：Seata 整个工作过程，由一个全局的事务ID，和TC事务管理者、TM事务协调者、RM资源管理器。每个阶段执行完，会存储执行前和执行后的快照信息到数据库表中。只要有一个阶段出现异常，就会回滚事务。回滚之前，判断当前存储的数据，与数据库中的数据对比，是否被修改过。如果被修改过，那么就回滚失败。只能人工干预处理。如果没有被修改过，那么就会把保存的快照数据改回到到数据库，然后删除本次事务的所有日志记录
